...
fn test_metrics_have_expected_ranges() {
    ...
    // Check GC content
    let gc_metric = frame.metric_ids().iter()
    ...
    if let Some(metric_id) = gc_metric {
        ...
        assert!(min_val >= 25.0 && max_val <= 80.0, "GC out of expected range");
    }
}
...
fn test_report_generation_with_synthetic_data() {
    ...
    assert!(content.contains("PopQC"), "Report should have PopQC branding");
